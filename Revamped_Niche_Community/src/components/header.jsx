  ...
  useEffect(() => {
    if (currentUser) {
      ...
      onValue(postsRef, (snapshot) => {
        if (snapshot.exists()) {
          const data = Object.entries(snapshot.val()).map(([id, post]) => ({
            id,
            ...post,
          }));
                ...
                >
                  ...
                  {notifications.length > 0 ? (
                    notifications.map((post) => (
                      <div
                        key={post.id}
                        className="p-2 border-b border-gray-600 cursor-pointer hover:bg-gray-600"
                        onClick={() => handleNotificationClick(post.id)}
                      >
                        <p className="text-xs text-gray-300">{post.content.substring(0, 100)}...</p>
                        <span className="text-gray-400 text-xs">{post.author}</span>
                      </div>
                    ))
                  ) : (
